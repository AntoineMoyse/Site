...
namespace Site.Areas.Identity.Pages.Account
{
    ...
    public class ExternalLoginModel : PageModel
    {
        ...
        public async Task<IActionResult> OnGetCallbackAsync(string returnUrl = null, string remoteError = null)
        {
            ...
            if (remoteError != null)
            {
                ErrorMessage = $"Erreur du prestataire externe: {remoteError}";
                return RedirectToPage("./Login", new {ReturnUrl = returnUrl });
            }
            var info = await _signInManager.GetExternalLoginInfoAsync();
            if (info == null)
            {
                ErrorMessage = "Erreur lors du chargement des informations de connection externe.";
                return RedirectToPage("./Login", new { ReturnUrl = returnUrl });
            }
            ...
            if (result.Succeeded)
            {
                _logger.LogInformation("{Name} connecté avec {LoginProvider}.", info.Principal.Identity.Name, info.LoginProvider);
        ...
        public async Task<IActionResult> OnPostConfirmationAsync(string returnUrl = null)
        {
            ...
            if (info == null)
            {
                ErrorMessage = "Erreur lors du chargement des informations de connection externe pendant la confirmation.";
                return RedirectToPage("./Login", new { ReturnUrl = returnUrl });
            }
            ...
            if (ModelState.IsValid)
            {
                ...
                if (result.Succeeded)
                {
                    ...
                    if (result.Succeeded)
                    {
                        _logger.LogInformation("L'utilisateur a créé un compte avec {Name}.", info.LoginProvider);
